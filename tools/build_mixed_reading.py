...
def payload(l=None):
    data={'mixedReading':True,'paths':[{k:p[k] for k in ['id','a','b']} for p in PATHS],'app':D['app'],'title':D['title'],'layoutIds':D['layoutIds'],'lessons':[{k:x[k] for k in ['id','slug','title','default']} for x in D['lessons']],'current':l['id'] if l else 0,'quiz':l['quiz'] if l else []}
...
def lab(l):
    out=shared.lab(l)
    panel='<section id="mr-diagnostics" class="mr-panel" aria-live="polite"><h3>Три языка в одном раскладе</h3><p id="mr-counts"></p><div class="mr-table-wrap" tabindex="0" role="region" aria-label="Состав и соответствия выбранных карт"><table class="mr-table"><caption>Карта, собственное соответствие и вопрос её места</caption><thead><tr><th scope="col">Карта</th><th scope="col">Группа</th><th scope="col">Соответствие</th><th scope="col">Вопрос позиции</th></tr></thead><tbody id="mr-families"></tbody></table></div><details class="sg-disclosure"><summary>Одинаковые числовые ранги в разных мастях</summary><div id="mr-repeats"></div></details><details class="sg-disclosure" open><summary>Пути и числовые соответствия их концов</summary><div id="mr-path-links"></div></details><p class="mr-reference">Здесь сопоставляются сефиротические функции выбранной герметической схемы. Разные масти сохраняют разные миры; совпадение ранга не создаёт буквальный переход между мирами. Придворные не считаются числовыми концами путей. Направление перехода и жизненный смысл Вы обосновываете отдельно.</p></section>'
    out=out.replace('<div id="sg-cards" class="sg-card-grid"></div>','<div id="sg-cards" class="sg-card-grid"></div>'+panel)
    out=out.replace('На какие детали, соответствия и источники я опираюсь','Основание: наблюдение / источник / соответствие / моя гипотеза')
    out=out.replace('<p id="sg-layout-help" class="sg-help"></p>','<p id="sg-layout-help" class="sg-help"></p><p class="fine">У исходного упражнения свои вопросы позиций. Их можно изменить в схемах «Своя схема». Выбор другой схемы задаёт её стандартные вопросы. Прежние записи доступны в «Моих других вариантах».</p>')
    return out
def notebook(page,crumbs):
    b=crumbs('../../../','Тетрадь смешанных раскладов')+'<div class="sg-module tr-module mr-module"><header class="sg-hero"><p class="eyebrow">Отдельная тетрадь модуля</p><h1>Мои смешанные расклады</h1><p class="lead">Варианты раскладов, источники Ваших объяснений, зафиксированные гипотезы и последующие наблюдения.</p>'+progress()+'<p>Записи остаются в этом браузере на этом устройстве. Автор курса их не получает. Скачайте JSON для резервной копии и переноса; TXT подходит для чтения.</p>'+shared.notes_controls()+'<p id="sg-save-status" role="status"></p><p><a href="../">Вернуться к программе →</a></p></header><section class="sg-section"><h2>Мои варианты</h2><label class="sg-field">Найти карту, вопрос или текст<input type="search" id="sg-search" disabled data-sg-enable></label><div id="sg-notes"></div><button type="button" class="button secondary" id="sg-more" hidden>Показать ещё</button></section><section id="observations" class="sg-section"><h2>Сохранённые разборы и возвращения</h2><p>Исходный снимок остаётся прежним. Новые факты и понимание добавляются отдельной записью с датой.</p><div id="sg-snapshots"></div><button type="button" class="button secondary" id="sg-snap-more" hidden>Показать ещё снимки</button></section><noscript><p>Для личных записей и переноса тетради включите JavaScript.</p></noscript></div>'+payload()
    page(BASE+'notebook/','Мои смешанные расклады — тетрадь','Личная тетрадь смешанных раскладов: основания, гипотезы и наблюдения.',b,noindex=True)
def build(page,crumbs):
    ...
    b=crumbs('../../','Смешанные расклады')+'<div class="sg-module tr-module mr-module"><header class="sg-hero"><p class="eyebrow">Следующая ступень · 6 занятий · 12 вопросов</p><h1>'+E(D['title'])+'</h1><p class="sg-subtitle">'+E(D['subtitle'])+'</p><p class="lead">'+E(D['lead'])+'</p><div class="sg-actions"><a class="button" href="light-and-vessel/">Начать со света и сосуда →</a><a class="button secondary" href="#program">Шесть занятий</a><a href="notebook/">Моя тетрадь</a></div>'+progress()+'</header><section class="sg-section sg-intro"><div><h2>Соединить изученное</h2><p>После <a href="../court-relations/">модуля о придворных</a> соединяем три языка колоды: число и сефиру, придворный способ участия, букву и путь Старшего аркана.</p><p>Новая опора — Пардес римоним о свете и сосудах, Игерет ха-кодеш о даровании и восприятии, Томер Двора о связи понимания с поступком.</p></div><div><h2>От таблицы к собственному чтению</h2><p>На занятие — 25–40 минут. Сначала прочитайте фрагмент в его контексте, затем исследуйте исходный пример или собственный вопрос. В лаборатории доступны все 78 карт и схемы из четырёх, пяти и шести позиций.</p><p>Справочная панель показывает группы карт, повторение числовых рангов и совпадения между рангами и концами путей. Отдельная тетрадь сохраняет Ваши объяснения, их основания и последующие наблюдения.</p></div></section><section class="sg-section"><h2>Три языка колоды</h2><div class="mr-table-wrap" tabindex="0" role="region" aria-label="Сопоставление трёх групп колоды"><table class="mr-table"><caption>Собственные соответствия в выбранной герметической системе</caption><thead><tr><th scope="col">Группа</th><th scope="col">Как устроено соответствие</th><th scope="col">Что уточняем в чтении</th></tr></thead><tbody><tr><th scope="row">40 числовых карт</th><td>Ранг / сефира × масть / мир</td><td>Как эта функция проявляется в данной области?</td></tr><tr><th scope="row">16 придворных</th><td>Стихия ранга внутри стихии масти</td><td>Каким способом участвовать в происходящем?</td></tr><tr><th scope="row">22 Старших аркана</th><td>Буква и путь между сефирот</td><td>Какое отношение или переход исследовать?</td></tr></tbody></table></div><p class="mr-reference">Свет, сосуд и передача — вопросы нового модуля. Они не закреплены за тремя группами колоды как неизменные роли. Любая карта отвечает на вопрос своей позиции, сохраняя собственное соответствие.</p></section><section id="program" class="sg-section"><h2>Маршрут занятий</h2><div class="sg-program">'
    ...
    b+='</div></section><section class="sg-section"><h2>Слова, с которыми будем работать</h2><dl class="tr-terms">'+''.join('<div><dt>'+E(k)+'</dt><dd>'+E(v)+'</dd></div>' for k,v in D['terms'])+'</dl></section>'+checklist()+sources()+'<section class="sg-section"><h2>Следующий модуль: мера и возвращение</h2><p>Как читать прямые и перевёрнутые карты, различать недостаток и избыток и возвращаться к собственным выводам с новыми наблюдениями.</p><a class="button" href="../reversal-study/">Перейти к модулю →</a></section>'+'</div>'+payload()
    page(BASE,D['title'],D['lead'],b)
    for l in D['lessons']:
        b=crumbs('../../../','Смешанный расклад · '+str(l['id']))+'<div class="sg-module tr-module mr-module"><p><a href="../">← Весь модуль</a> · <a href="../notebook/">Моя тетрадь</a></p><header class="sg-lesson-hero"><p class="eyebrow">Занятие '+str(l['id'])+' из 6</p><h1>'+E(l['title'])+'</h1><p class="lead">'+E(l['aim'])+'</p>'+progress()+'</header><nav class="sg-lesson-nav" aria-label="Разделы занятия"><a href="#reading">Источник</a><a href="#explanation">Разбор</a><a href="#task">Задание</a><a href="#laboratory">Лаборатория</a><a href="#check">Самопроверка</a></nav>'+reading(l)+'<div id="explanation" class="sg-reading"><p class="sg-opening">'+E(l['opening'])+'</p>'
        for s in l['sections']:b+='<section><h2>'+E(s['title'])+'</h2>'+''.join(p(t) for t in s['text'])+'</section>'
        b+='</div>'
        b+='<section id="task" class="sg-section"><h2>Попробуйте сами</h2><ol>'+''.join('<li>'+E(t)+'</li>' for t in l['task'])+'</ol>'+p('Исходные карты: '+' → '.join(shared.BY_ID[id]['name'] for id in l['default']['ids']))+shared.example(l)+passport(l)+'</section>'+lab(l)
        ...
        if l['id']==N:b+=checklist()+'<section class="sg-section"><h2>Следующий модуль: мера и возвращение</h2><p>Как читать прямые и перевёрнутые карты, различать недостаток и избыток и возвращаться к собственным выводам с новыми наблюдениями.</p><a class="button" href="../../reversal-study/">Перейти к модулю →</a></section>'
        ...
        page(BASE+l['slug']+'/',l['title']+' — смешанные расклады',l['aim'],b)
